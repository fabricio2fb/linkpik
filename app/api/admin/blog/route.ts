...
export async function POST(request: Request) {
  try {
    ...
    const { data: existing } = await supabase.from("blog_posts").select("id").eq("slug", parsed.data.slug).maybeSingle();
    if (existing) throw new ApiError(409, "Ja existe um post com este slug");
    ...
    if (error) throw new ApiError(500, "Erro ao criar post");

    await logAdminAction({
      action: "criou_post_blog",
