...
export async function PATCH(request: Request) {
  try {
    ...
    const { data: links } = await supabase.from("links").select("id").in("id", ids).eq("creator_id", creator.id);
    if (!links || links.length !== ids.length) {
      throw new ApiError(403, "Um ou mais links nao pertencem a este creator");
    }

    for (const item of parsed.data.items) {
      await supabase.from("links").update({ position: item.position }).eq("id", item.id).eq("creator_id", creator.id);
    }
